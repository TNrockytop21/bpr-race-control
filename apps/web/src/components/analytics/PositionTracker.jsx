/**
 * Position Tracker — shows position changes over time.
 * P1 at top, step-line style. Who gained, who fell.
 * OBS overlay ready.
 */
...
export function PositionTracker({ driverFilter, maxDrivers = 10 }) {
  ...
  const chartData = useMemo(() => {
    ...
    return gapHistory.map((snap) => {
      ...
      for (const name of driverNames) {
        point[name] = snap.drivers[name]?.pos ?? null;
      }
      return point;
    });
  ...
  const maxPos = useMemo(() => {
    let max = 10;
    for (const snap of gapHistory) {
      for (const d of Object.values(snap.drivers)) {
      ...
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
          ...
          <YAxis
            ...
            reversed
            domain={[1, maxPos]}
            label={{ value: 'Position', angle: -90, position: 'insideLeft', style: { fontSize: 9, fill: '#666' } }}
          />
          <Tooltip
            contentStyle={{ background: '#111', border: '1px solid #333', borderRadius: '4px', fontSize: '11px' }}
            labelStyle={{ color: '#888' }}
            labelFormatter={(v) => `${v} min`}
            formatter={(v) => v != null ? [`P${v}`] : ['--']}
          />
